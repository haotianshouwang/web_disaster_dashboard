(() => {
    ...
    /**
     * 解析 WebSocket 基础 URL：
     * 开发模式下可通过 window.__WS_BASE_URL__ 配置后端地址（如 ws://127.0.0.1:8089），
     * 也可通过 window.__API_BASE_URL__ 自动推导（将 http:// 替换为 ws://），
     * 未配置时默认使用当前页面同源地址。
     */
    function resolveWsOrigin() {
        if (window.__WS_BASE_URL__ && typeof window.__WS_BASE_URL__ === 'string') {
            return window.__WS_BASE_URL__.replace(/\/+$/, '');
        }
        if (window.__API_BASE_URL__ && typeof window.__API_BASE_URL__ === 'string') {
            return window.__API_BASE_URL__.replace(/\/+$/, '').replace(/^http/, 'ws');
        }
     ...
     */
    function getWsUrl() {
        // 仪表盘使用 /dashboard/ws 端点 + 独立密钥
        const key = (window.dashboardKey) || '';
        const keyParam = key ? `?key=${encodeURIComponent(key)}` : '';
        return `${resolveWsOrigin()}/dashboard/ws${keyParam}`;
    }
