...
function CalendarHeatmap({ style, className = '' }) {
    ...
    const years = useMemo(function () {
        var arr = [];
        for (var y = startYear; y <= currentYear; y++) arr.push(y);
        return arr.reverse();
    }, [currentYear]);

    // 从 DashboardData 获取热力图数据
    useEffect(function () {
        setLoading(true);
        var update = function () {
            var store = window.DashboardData.getStore();
            var heatmap = Array.isArray(store.heatmap) ? store.heatmap : [];
            setData(heatmap);
            setLoading(false);
        };
        update();
    ...
    useEffect(function () {
        if (!lastEvent) return;
        var eventTime = new Date(lastEvent.time || lastEvent.event_time || Date.now());
